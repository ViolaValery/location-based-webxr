...
export default defineConfig(
  ...
  // Shared project defaults
  {
    ...
    rules: {
      ...
      complexity: ["warn", 10],
      "max-depth": ["warn", 4],
      // The starter consumes the closed-source core library only via the
      // curated `gps-plus-slam-app-framework` re-exports — it must never take a
      // direct dependency on `gps-plus-slam-js` (the framework curates that
      // package's public surface for app consumers). Mirrors the RecorderApp
      // boundary so all example apps share one import convention.
