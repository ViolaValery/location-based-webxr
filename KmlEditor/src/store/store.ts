...
// IMPORTANT: keep redux-undo's default undoType/redoType here. The
// `undo`/`redo` action creators exported below (from redux-undo's
// ActionCreators) always dispatch the library's default action types —
// overriding undoType/redoType in the config below would silently break
// them, since the reducer would then only respond to the custom types.
